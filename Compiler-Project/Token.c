...
Token *back_token() 
{ 			
	// case 1:		If only one node in linked list and currentIndex == 0 then return NULL.
	// case 2:		If only one node in linked list and currentIndex > 0 then return token from [currentIndex].
	// case 3:		If linked list has more than one node and currentIndex == 0 then return last token from previous node.
	// case 4:		If linked list has more than one node and currentIndex > 0 then return token from [currentIndex].
	
	// Case 1
	Token *token = NULL;

	// Case 3	
	if (currentNode->prev != NULL && currentIndex == 0) 
	{
...
{ 	
	...
	// If back_token function was not called continue with lexical analysis.
	if(back_token_called == 0)
	{		
		yylex();
		token = &currentNode->tokensArray[currentIndex];		
	}
	// Use tokens from the storage and not start lexical analysis.
	else 
	{		
		currentIndex++;

		if(currentIndex == TOKEN_ARRAY_SIZE - 1 && currentNode->next != NULL) 			
		{
			currentIndex = 0;
			token = &currentNode->next->tokensArray[currentIndex];
		}
		else if(currentIndex > 0 && currentIndex < TOKEN_ARRAY_SIZE)
		{
			token = &currentNode->tokensArray[currentIndex];
		}	
